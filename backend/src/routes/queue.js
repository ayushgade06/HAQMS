...
router.post('/checkin', authenticate, async (req, res) => {
  try {
    ...
    //   data: { tokenNumber: nextTokenNumber, patientId, doctorId, appointmentId: appointmentId || null, status: 'WAITING' },
    //   include: { patient: true, doctor: true },
    // });

    // Serializable transaction: the DB serializes concurrent reads of the same token sequence,
    // so no two transactions can read the same max and write the same next token.
